...
if __name__ == '__main__':
    ...
    rev_noise_ratio = 0.1
    theta0 = 1
    xs, theta_hats, errors = simulate_sk(num_iter, snr, rev_noise_ratio,
                                         theta0)

    t = np.arange(num_iter)
    plt.plot(t, xs)
    plt.plot(t, theta0 * np.ones(t.size), 'k--', linewidth=2)
    plt.errorbar(t, theta_hats, errors)
    plt.title('Evolution of $X_i$ and $\hat{\Theta}_i$\nError is 2-$\sigma$\n'
              r'$\sigma_r^2/\sigma_n^2 = %f,\,\sigma_\theta^2/\sigma_n^2 = %f$'
              % (rev_noise_ratio ** 2, snr ** 2))
