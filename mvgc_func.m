function [F, success] = mvgc_func(X, ntrials, nobs)
%% MVGC demo
%
...
alpha     = 0.5;   % significance level for significance test
...
acerr = var_info(info, false); % report results (and bail out on error)
if acerr
	F = [nan, nan; nan, nan];
	success = false;
	return;
